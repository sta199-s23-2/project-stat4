...
=======
>>>>>>> 345858fe51e7d5608b8da988cb6a23b89cb6d856
```{r}
...
linear_reg() |>
  set_engine ("lm") |>
  fit (proportion ~ proportion_vechicle, data = Midwest) |>
  glance() |>
  pull (adj.r.squared)
```
...
After seeing the results to our hypothesis test and CI, we believe that the Midwest is more impacted by low income and being 10 miles away. We used a linear regression to see which variables affect that. We choose the variables proportion of seniors and no vehicle access being 10 miles away. First, we fit those variables individually. Then we looked at their adjusted r squared values. The linear regression with the proportion of seniors had a higher r-squared. So then we added a additive and interactive component with the proportion of having no vehicle access. The interative model had a higher r squared value. 
...
Linear regression: We created a linear regression to see which variables effect low income and being 10 miles away for the Midwest. We found that the best model was when the proportion of seniors and proportion of no vehicle access being 10 miles away interacted with each other. This model had the highest adjusted r squared value (0.8715612). A higher r-squared value indicates a higher amount of variability being explained by our model. Therefore, our r-squared value supports that our interactive model is a good fit for our data. This value answers the second part of our research question. Individiuals who are a senior and have no vehicle access and being 10 miles beyond a supermarket contribute to low income and being 10 miles beyond a supermarket in the midwest. 
